...
jsi::Value JsiHostObject::get(jsi::Runtime& runtime, const jsi::PropNameID& name) {
  ...
  if (func != funcs.end()) {
    std::map<std::string, jsi::Function>& runtimeCache = _hostFunctionCache.get(runtime);
  ...
  if (func != funcs.end()) {
    ...
    return _hostFunctionCache.get(runtime)
        .emplace(nameStr, jsi::Function::createFromHostFunction(runtime, name, 0, dispatcher))
        .first->second.asFunction(runtime);
  }

  if (_funcMap.count(nameStr) > 0) {
    return jsi::Function::createFromHostFunction(runtime, name, 0, _funcMap.at(nameStr));
  }
