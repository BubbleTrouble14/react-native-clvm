...
#define EXPORT_JSI_API_TYPENAME(CLASS, TYPENAME) \
  ...
  template <typename T>
  class JsiClvmWrappingHostObject : public RNJsi::JsiHostObject
  {
  public:
    /**
     * Default constructor
     ...
     */
    JsiClvmWrappingHostObject(T object) : RNJsi::JsiHostObject(), _object(std::move(object)) {}
  ...
  template <typename T, typename HostObjectType>
  class JsiClvmWrappingSharedPtrHostObject : public JsiClvmWrappingHostObject<std::shared_ptr<T>>
  {
  public:
    JsiClvmWrappingSharedPtrHostObject(std::shared_ptr<T> object) : JsiClvmWrappingHostObject<std::shared_ptr<T>>(std::move(object)) {}
    ...
    //   return std::static_pointer_cast<JsiClvmWrappingSharedPtrHostObject>(obj.asObject(runtime).asHostObject(runtime))->getObject();
    // }
  ...
  {
  public:
    JsiCLVMObjectWrapper(chia::CLVMObjectPtr object)
        : JsiClvmWrappingHostObject<chia::CLVMObjectPtr>(std::move(object)) {}

    // /**
    //   Returns the underlying object from a host object of this type
    //  */
    // static chia::CLVMObjectPtr fromValue(jsi::Runtime &runtime, const jsi::Value &obj)
    // {
    //   return std::static_pointer_cast<JsiCLVMObjectWrapper>(
    //              obj.asObject(runtime).asHostObject(runtime))
    //       ->getObject();
    // }

    // Returns the jsi object from a T object
    // Corrected toValue method for JsiCLVMObjectWrapper
    static jsi::Value toValue(jsi::Runtime &runtime, chia::CLVMObjectPtr object)
    {
      auto wrapper = std::make_shared<JsiCLVMObjectWrapper>(std::move(object));
